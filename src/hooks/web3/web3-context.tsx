...
export const Web3ContextProvider: React.FC<{ children: ReactElement }> = ({ children }) => {
  ...
  const mainnet = Number(DEFAULT_NETWORK)

  const [uri, setUri] = useState(getMainnetURI(mainnet))
    ...
    new Web3Modal({
      ...
      providerOptions: {
        walletconnect: {
          ...
          options: {
            rpc: {
              [mainnet ? Networks.BSC : Networks.BSC_TESTNET]: getMainnetURI(mainnet),
            },
          },
        },
      },
  ...
  const checkWrongNetwork = async (): Promise<boolean> => {
    if (providerChainID !== DEFAULT_NETWORK) {
      if (!mobile) {
        ...
        disconnect()
        setProviderChainID(DEFAULT_NETWORK)
        connect()
      }
