...
function useBonds() {
  const bondLoading = useSelector<AppState, boolean>((state) => state.bonding.loading)
  const bondState = useSelector<AppState, IBondSlice>((state) => state.bonding)

  const accountBondsState = useSelector<AppState, { [key: string]: IUserBondDetails }>((state) => state.account.bonds)
  //@ts-ignore
