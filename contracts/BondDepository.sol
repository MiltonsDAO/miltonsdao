...
contract MlsBondDepository is Ownable {
    ...
    IERC20 public immutable Mls; // token given as payment for bond
    IERC20 public immutable principle; // token used to create bond
    ITreasury public immutable treasury; // mints Mls when receives principle
    ...
    struct Bond {
        uint256 payout; // Mls remaining to be paid
    ...
    function setReferralAddress(address _addressReferral) external onlyOwner {
    ...
    function deposit(
        ...
        uint256 value = treasury.riskFreeValueOf(address(principle), _amount);
        uint256 payout = payoutFor(value); // payout to bonder is computed
        require(totalDebt.add(value) <= terms.maxDebt, "Max capacity reached");
        require(payout >= 10000000, "Bond too small"); // must be > 0.01 Mls ( underflow protection )
        ...
        /** principle is transferred in approved and deposited into the treasury, returning (_amount - profit) Mls */
    ...
    function stakeOrSend(
        ...
        if (!_stake) {
            Mls.transfer(_recipient, _amount); // send payout
        } else {
            if (useHelper) {
                Mls.approve(address(stakingHelper), _amount);
                stakingHelper.stake(_amount, _recipient);
            } else {
                Mls.approve(address(staking), _amount);
    ...
    function maxPayout() public view returns (uint256) {
        return Mls.totalSupply().mul(terms.maxPayout) / 100000;
    }

    function payoutFor(uint256 _value) public view returns (uint256) {
        return
            FixedPoint.fraction(_value, bondPrice()).decode112with18() / 1e16;
    }
    ...
    function debtRatio() public view returns (uint256 debtRatio_) {
        uint256 supply = Mls.totalSupply();
        debtRatio_ =
            FixedPoint
    ...
    function standardizedDebtRatio() external view returns (uint256) {
        if (isLiquidityBond) {
            return
                debtRatio().mul(bondCalculator.markdown(address(principle))) /
                1e9;
        } else {
            return debtRatio();
        }
