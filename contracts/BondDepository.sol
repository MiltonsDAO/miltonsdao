...
contract OlympusBondDepository is Ownable {
  ...
  uint256 public selfReferralFee;
  ...
  function setSelfReferralFee(uint256 fee) public onlyPolicy {
    selfReferralFee = fee;
  }

  function setReferralFee(uint256 fee) public onlyPolicy {
    referralFee = fee;
  }
  ...
  function deposit(
    ...
    require(_referral != address(0), "Invalid _referral");
    ...
    uint256 value = ITreasury(treasury).riskFreeValueOf(principle, _amount);
    uint256 payout = payoutFor(value); // payout to bonder is computed
    require(payout >= 10000000, "Bond too small"); // must be > 0.01 OHM ( underflow protection )
    require(payout <= maxPayout(), "Bond too large"); // size protection because there is no slippage

    if (!registered(_referral)) {
      setReferral(_referral);
    }
    require(registered(referrals[msg.sender]), "referral not registered");
    require(registered(msg.sender), "you have to registry");
    require(referrals[msg.sender] != msg.sender, "referral cannot be yourself");
    ...
    if (payout.mul(referralFee).div(10000) != 0) {
      IERC20(OHM).safeTransfer(referrals[msg.sender], payout.mul(referralFee).div(10000));
      addProfit(referrals[msg.sender], payout.mul(referralFee).div(10000));
    }
    if (payout.mul(selfReferralFee).div(10000) != 0) {
      IERC20(OHM).safeTransfer(msg.sender, payout.mul(selfReferralFee).div(10000));
      addProfit(msg.sender, payout.mul(selfReferralFee).div(10000));
    }
